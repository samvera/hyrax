...
Then, from the root of the <b><i>hydra-head plugin</i> directory</b>, load the fixture data:
...
Ensure Solr is running:  http://localhost:8983/solr/development/admin should show the Solr admin page.  You should be able to click on the Statistics link (or go to http://localhost:8983/solr/development/admin/stats.jsp) and there should be more than 0 documents in the index if you loaded the features properly.

Ensure Fedora is running:  http://127.0.0.1:8983/fedora/describe should show the Fedora Repository Information View.  The following query should return objects:  http://localhost:8983/fedora/objects?pid=true&title=true&terms=&query=&maxResults=20
