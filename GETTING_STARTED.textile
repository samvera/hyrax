...
(need spec_helper to work at app level:  ruby script/generate rspec)


h3. Easy changes for practice.

h5. Overriding the helper

h6. (1) Write the test.

The application name is set in app/helpers/application_helper.rb.  So we will have a spec for the application_helper file.

File "application_helper_spec.rb" should have this:
...
Add a test for checking the application name might be inserted like so:
...
h6. (2) Change the code

Now you need to change the local app:

The application name is set in app/helpers/application_helper.rb.  The local app's application_helper file may look like this:
...
Now when you run the spec, the test should pass.



h5. Changing the Home Text
...
WRITE TESTS HERE!  THEY SHOULD SHOW WHAT THE END GOALS ARE ...
...
CAN WE DO TESTING NOW?  OR DO WE NEED TO BUNDLE IN THE RIGHTS METADATA BEFORE TESTING?

h4. rightsMetadata -- The Hydra Rights Metadata Datastream

Hydra provides a class definition for the rightsMetadata datastream, so you won't have to define the Terminology yourself.  The definition is in the hydra-head code in "lib/hydra/rights_metadata.rb":https://github.com/projecthydra/hydra-head/blob/master/lib/hydra/rights_metadata.rb lib/hydra/rights_metadata.rb.
