...
module Hyrax
  ...
  class AdminSetCreateService # rubocop:disable Metrics/ClassLength
    ...
    class << self
      ...
      # @param id [Hyrax::AdministrativeSet | AdminSet] the id of the admin set to check
      def default_admin_set?(admin_set:)
        ...
        # For any other Valkyrie Adapter, the DEFAULT_ID will be an alternate id.
      ...
      # Is the admin_set the default Hyrax::AdministrativeSet
      # @param id [Hyrax::AdministrativeSet | AdminSet] the id of the admin set to check
      def default_admin_set_id?(id:)
        default_admin_set_id == id.to_s
      end
      ...
      def default_admin_set_id
        default_admin_set = find_or_create_default_admin_set
      ...
      def call(admin_set:, creating_user:, **kwargs)
        ...
        raise err if default_admin_set?(admin_set: admin_set)
        false
      end
      ...
      # @raise [RuntimeError] if admin set cannot be persisted
      def call!(admin_set:, creating_user:, **kwargs)
        raise "Use .find_or_create_default_admin_set to create a default admin set" if default_admin_set?(admin_set: admin_set)
    ...
    def default_admin_set?
      self.class.default_admin_set?(admin_set: admin_set)
    end
