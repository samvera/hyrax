RSpec.describe CatalogController, type: :controller do
  ...
  describe "#index" do
    ...
    context 'works by file metadata' do
      ...
      let(:work1) do
        { Valkyrie::Persistence::Solr::Queries::MODEL => ["GenericWork"], id: "ff365c76z", title_tesim: ["me too"],
          file_set_ids_ssim: ["ff365c78h", "ff365c79s"],
          read_access_group_ssim: ["public"], edit_access_person_ssim: ["user1@example.com"] }
      end

      let(:work2) do
        { Valkyrie::Persistence::Solr::Queries::MODEL => ["GenericWork"], id: "ff365c777", title_tesim: ["find me"],
          file_set_ids_ssim: [],
          read_access_group_ssim: ["public"], edit_access_person_ssim: ["user2@example.com"] }
      end

      let(:file1) do
        { Valkyrie::Persistence::Solr::Queries::MODEL => ["FileSet"], id: "ff365c78h", title_tesim: ["find me"],
          file_set_ids_ssim: [],
          edit_access_person_ssim: [user.user_key] }
      end

      let(:file2) do
        { Valkyrie::Persistence::Solr::Queries::MODEL => ["FileSet"], id: "ff365c79s", title_tesim: ["other file"],
          file_set_ids_ssim: [],
          edit_access_person_ssim: [user.user_key] }
      end
