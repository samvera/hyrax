...
RSpec.describe Hyrax::CollectionsController, clean_repo: true do
  ...
  describe '#redirect_to_display_url' do
    ...
    context 'when the collection has a display URL' do
      before { Hyrax::RedirectPath.create!(path: '/awesome-collection', resource_id: collection.id.to_s, display_url: true) }
    ...
    context 'when the redirects feature is inactive' do
      before do
        ...
        Hyrax::RedirectPath.create!(path: '/awesome-collection', resource_id: collection.id.to_s, display_url: true)
      end
