...
describe BatchController do
  ...
  describe "#update" do
    before do
      ...
      @file.save
      @file2 = GenericFile.create(:batch=>@batch)
      # don't apply depositor metadata to @file2 ...
    end
    ...
    describe "when user has edit permissions on a file" do
      ...
      it "should set metadata like title" do
        post :update, :id=>@batch.pid, "generic_file"=>{"tag"=>["footag", "bartag"], "title"=>"New Title"} 
        file = GenericFile.find(@file.pid)
    ...
    describe "when user does not have edit permissions on a file" do
      it "should not modify the object" do
        ...
        file.save
        post :update, :id=>@batch.pid, "generic_file"=>{"read_groups_string"=>"group1, group2", "read_users_string"=>"", "tag"=>[""], "title"=>"New Title"} 
        file = GenericFile.find(@file2.pid)
