...
describe CurationConcerns::GenericWorksController do
  ...
  context "JSON" do
    ...
    describe 'failed update' do
      before { post :update, id: resource, generic_work: { title: nil, depositor: nil }, format: :json }
      it "returns 422 and the errors" do
        expect(response).to respond_unprocessable_entity(errors: { "edit_users": ["Depositor must have edit access"], "title": ["Your work must have a title."] })
      end
    end
  end
end
