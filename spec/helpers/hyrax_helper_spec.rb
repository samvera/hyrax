...
RSpec.describe HyraxHelper, type: :helper do
  ...
  describe "#collection_thumbnail" do
    let(:document) { SolrDocument.new(Valkyrie::Persistence::Solr::Queries::MODEL => ['Collection']) }
  ...
  describe "#iconify_auto_link" do
    ...
    let(:document)          { SolrDocument.new(Valkyrie::Persistence::Solr::Queries::MODEL => ['GenericWork'], id: 512, title_tesim: text, description_tesim: text) }
