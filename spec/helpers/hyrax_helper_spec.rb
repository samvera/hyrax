...
RSpec.describe HyraxHelper, type: :helper do
  ...
  describe "#link_to_each_facet_field" do
    ...
    context "with :output_separator" do
      let(:field) { ActiveFedora.index_field_mapper.solr_name("document_types", :facetable).to_sym }
      let(:options) { { config: { helper_facet: field, output_separator: ' ~ ', separator: ":" }, value: ["Imaging : Object Photography"] } }
