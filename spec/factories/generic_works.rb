FactoryBot.define do
  factory :work, aliases: [:generic_work], class: GenericWork do
    ...
    factory :work_with_files do
      before(:create) { |work, evaluator| 2.times { work.member_ids << create_for_repository(:file_set, user: evaluator.user).id } }
    end
    ...
    factory :with_embargo_date do
      ...
      factory :embargoed_work_with_files do
        after(:build) { |work, evaluator| work.apply_embargo(evaluator.embargo_date, evaluator.current_state, evaluator.future_state) }
        after(:create) { |work, evaluator| 2.times { work.member_ids << create_for_repository(:file_set, user: evaluator.user).id } }
      end
    end
