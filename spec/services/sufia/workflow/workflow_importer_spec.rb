...
RSpec.describe Sufia::Workflow::WorkflowImporter do
  ...
  context 'data generation' do
    ...
    it 'creates the requisite data from the configuration' do
      expect(Sufia::Workflow::WorkflowPermissionsGenerator).to receive(:call).and_call_original
      expect(Sufia::Workflow::SipityActionsGenerator).to receive(:call).and_call_original
      result = nil
      expect do
