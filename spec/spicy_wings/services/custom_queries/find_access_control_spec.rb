...
RSpec.describe SpicyWings::CustomQueries::FindAccessControl do
  subject(:query_handler) { described_class.new(query_service: query_service) }
  let(:adapter)           { Valkyrie::MetadataAdapter.find(:spicy_wings_adapter) }
  ...
  describe '#find_access_control' do
    ...
    context 'when the resource has been created via SpicyWings' do
