...
RSpec.describe SpicyWings::CollectionBehavior do
  ...
  describe '#add_collections_and_works' do
    ...
    context 'when new_member_ids are valkyrie ids' do
      let(:collection_resource2) { SpicyWings::ModelTransformer.new(pcdm_object: collection2).build }
      let(:collection_resource3) { SpicyWings::ModelTransformer.new(pcdm_object: collection3).build }
      let(:work_resource1) { SpicyWings::ModelTransformer.new(pcdm_object: work1).build }
      let(:work_resource2) { SpicyWings::ModelTransformer.new(pcdm_object: work2).build }
