...
RSpec.describe SpicyWings::Valkyrie::QueryService do
  before do
    module Hyrax::Test
      module QueryService
  ...
  let(:adapter) { SpicyWings::Valkyrie::MetadataAdapter.new }
  let(:persister) { SpicyWings::Valkyrie::Persister.new(adapter: adapter) }
  let(:af_resource_class) { Hyrax::Test::QueryService::Book }
  let(:af_image_resource_class) { Hyrax::Test::QueryService::Image }
  let(:resource_class) { SpicyWings::OrmConverter.to_valkyrie_resource_class(klass: af_resource_class) }
  let(:image_resource_class) { SpicyWings::OrmConverter.to_valkyrie_resource_class(klass: af_image_resource_class) }
  ...
  describe ".find_members" do
    ...
    context "filtering by model" do
      ...
      let(:parent_resource_class) { SpicyWings::OrmConverter.to_valkyrie_resource_class(klass: gw_resource_class) }
  ...
  describe ".find_inverse_references_by" do
    context "when the resource is saved" do
      ...
      # not yet supported by the spicy_wings persister
      context "when the property is ordered" do
