describe Sufia::WorkShowPresenter do
  ...
  describe "#permission_badge" do
    let(:badge) { instance_double(Sufia::PermissionBadge) }
    before do
      allow(Sufia::PermissionBadge).to receive(:new).and_return(badge)
    end
  ...
  describe "#attribute_to_html" do
    ...
    context 'with an existing field' do
      before do
        allow(Sufia::Renderers::AttributeRenderer).to receive(:new)
          .with(:title, ['foo', 'bar'], {})
          .and_return(renderer)
      end
    ...
    context "with a field that doesn't exist" do
      it "logs a warning" do
        expect(Rails.logger).to receive(:warn).with('Sufia::WorkShowPresenter attempted to render restrictions, but no method exists with that name.')
  ...
  context "with workflow" do
    ...
    describe "#workflow" do
      ...
      it { is_expected.to be_kind_of Sufia::WorkflowPresenter }
    end
  end
