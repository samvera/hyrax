...
RSpec.describe Hyrax::Transactions::Steps::RemoveRedirectPaths do
  ...
  describe '#call' do
    context 'when the resource has rows in the table' do
      before do
        Hyrax::RedirectPath.create!(source_path: '/handle/1', target_path: '/handle/1', resource_id: resource_id)
        Hyrax::RedirectPath.create!(source_path: '/handle/2', target_path: '/handle/2', resource_id: 'other-record')
      end
