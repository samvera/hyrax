...
describe Sufia::Arkivo::CreateSubscriptionJob do
  ...
  context 'with a bogus user' do
    ...
    it 'raises because user not found' do
      expect { described_class.perform_now(user.user_key) }.to raise_error(Sufia::Arkivo::SubscriptionError, 'User not found')
    end
  end
  ...
  context 'without an arkivo token' do
    ...
    it 'raises because user lacks arkivo token' do
      expect { described_class.perform_now(user.user_key) }.to raise_error(Sufia::Arkivo::SubscriptionError, 'User does not have an Arkivo token')
    end
  end

  context 'without a zotero userid' do
    it 'raises because user did not oauth' do
      expect { described_class.perform_now(user.user_key) }.to raise_error(Sufia::Arkivo::SubscriptionError, 'User has not yet connected with Zotero')
    end
  end
  ...
  context 'with an existing subscription' do
    ...
    it 'raises because user already has subscription' do
      expect { described_class.perform_now(user.user_key) }.to raise_error(Sufia::Arkivo::SubscriptionError, 'User already has a subscription')
    end
  end
