...
describe Sufia::Arkivo::Actor do
  ...
  describe '#create_work_from_item' do
    ...
    it 'instantiates an actor' do
      expect(Sufia::Actors::GenericWorkActor).to receive(:new).once.and_call_original
      subject.create_work_from_item
    end

    it 'creates initial metadata' do
      expect_any_instance_of(Sufia::Actors::FileSetActor).to receive(:create_metadata).once
      subject.create_work_from_item
    end
    ...
    it 'calls create_content' do
      expect_any_instance_of(Sufia::Actors::FileSetActor).to receive(:create_content).once
      subject.create_work_from_item
    end
  ...
  describe '#update_work_from_item' do
    ...
    it 'instantiates an actor' do
      expect(Sufia::Actors::FileSetActor).to receive(:new).once.and_call_original
    ...
    it 'calls update_content' do
      expect_any_instance_of(Sufia::Actors::FileSetActor).to receive(:update_content).once
      subject.update_work_from_item(work)
    end
