...
RSpec.describe Sufia::Forms::WorkflowActionForm, no_clean: true do
  ...
  context 'if the given user cannot perform the given action' do
    before { expect(Sufia::Workflow::PermissionQuery).to receive(:authorized_for_processing?).and_return(false) }
    describe '#valid?' do
      subject { form.valid? }
    ...
    describe '#save' do
      ...
      it 'will not send the #deliver_on_action_taken message to Sufia::Workflow::NotificationService' do
        expect(Sufia::Workflow::NotificationService).not_to receive(:deliver_on_action_taken)
        subject
      end

      it 'will not send the #handle_action_taken message to Sufia::Workflow::ActionTakenService' do
        expect(Sufia::Workflow::ActionTakenService).not_to receive(:handle_action_taken)
        subject
      end
    end
  end
  ...
  context 'if the given user can perform the given action' do
    before do
      expect(Sufia::Workflow::PermissionQuery).to receive(:authorized_for_processing?)
        .and_return(true)
    end
    ...
    describe '#save' do
      ...
      it 'will send the #deliver_on_action_taken message to Sufia::Workflow::NotificationService' do
        expect(Sufia::Workflow::NotificationService).to(
          receive(:deliver_on_action_taken).with(entity: sipity_entity, comment: kind_of(Sipity::Comment), action: an_action, user: user)
        )
        subject
      end

      it 'will send the #handle_action_taken message to Sufia::Workflow::ActionTakenService' do
        expect(Sufia::Workflow::ActionTakenService).to(
