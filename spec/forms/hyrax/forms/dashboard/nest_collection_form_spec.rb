...
RSpec.describe Hyrax::Forms::Dashboard::NestCollectionForm, type: :form do
  ...
  ["Solr graph is on", "Solr graph is off"].each do |graph_status|
    context "when parent/child are ActiveFedora object and #{graph_status}" do
      subject(:form) do
      ...
      before do
        allow(Hyrax.config).to receive(:use_solr_graph_for_collection_nesting).and_return(true) if graph_status == "Solr graph is on"
      end
    ...
    context "when receiving parent/child Valkyrie IDs and #{graph_status}" do
      subject(:form) do
