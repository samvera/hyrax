...
RSpec.describe 'collection', type: :feature, index_adapter: :solr_index, valkyrie_adapter: :postgres_adapter do
  ...
  let(:admin_set_a) { FactoryBot.valkyrie_create(:hyrax_admin_set, creator: [admin_user.user_key], title: ['Set A'], with_permission_template: true) }
  let(:admin_set_a_permission_template) { Hyrax::PermissionTemplate.find_by(source_id: admin_set_a.id.to_s) }
  let(:admin_set_b) { FactoryBot.valkyrie_create(:hyrax_admin_set, creator: [user.user_key], title: ['Set B'], edit_users: [user.user_key], with_permission_template: true) }
  let(:admin_set_b_permission_template) { Hyrax::PermissionTemplate.find_by(source_id: admin_set_b.id.to_s) }
  let(:collection1) { FactoryBot.valkyrie_create(:hyrax_collection, :public, user: user, collection_type_gid: collection_type.gid.to_s, with_permission_template: true) }
  let(:collection1_permission_template) { Hyrax::PermissionTemplate.find_by(source_id: collection1.id.to_s) }
  let(:collection2) { FactoryBot.valkyrie_create(:hyrax_collection, :public, user: user, collection_type_gid: collection_type.gid.to_s, with_permission_template: true) }
  let(:collection2_permission_template) { Hyrax::PermissionTemplate.find_by(source_id: collection2.id.to_s) }
  let(:collection3) { FactoryBot.valkyrie_create(:hyrax_collection, :public, user: admin_user, collection_type_gid: collection_type.gid.to_s, with_permission_template: true) }
  let(:collection3_permission_template) { Hyrax::PermissionTemplate.find_by(source_id: collection3.id.to_s) }
  let(:collection4) { FactoryBot.valkyrie_create(:hyrax_collection, :public, user: admin_user, collection_type_gid: user_collection_type.gid.to_s, with_permission_template: true) }
  let(:collection4_permission_template) { Hyrax::PermissionTemplate.find_by(source_id: collection4.id.to_s) }

  before do
    # Set this to ensure that searches are looking for the collection model these tests are creating
    allow(Hyrax.config).to receive(:collection_model).and_return('Hyrax::PcdmCollection')
  end

  describe 'Your Collections tab', skip: "pending" do
    context 'when non-admin user' do
      before do
        ...
        FactoryBot.create(:permission_template_access,
               :manage,
               permission_template: admin_set_b_permission_template,
               agent_type: 'user',
    ...
    context 'when admin user' do
      before do
        ...
        admin_set_a
        admin_set_b
        FactorBot.create(:permission_template_access,
               :manage,
               permission_template: admin_set_a_permission_template,
               agent_type: 'user',
  ...
  describe 'Managed Collections tab (for non-admin users with shared access' do
    ...
    before do
      ...
      FactoryBot.create(:permission_template_access,
             :manage,
             permission_template: collection1_permission_template,
             agent_type: 'user',
             agent_id: user2.user_key)
      collection1_permission_template.reset_access_controls_for(collection: collection1, interpret_visibility: true)
      FactoryBot.create(:permission_template_access,
             :deposit,
             permission_template: Hyrax::PermissionTemplate.find_by(source_id: collection2.id.to_s),
             agent_type: 'user',
             agent_id: user2.user_key)
      collection2_permission_template.reset_access_controls_for(collection: collection2, interpret_visibility: true)
      FactoryBot.create(:permission_template_access,
             :view,
             permission_template: Hyrax::PermissionTemplate.find_by(source_id: collection4.id.to_s),
             agent_type: 'user',
             agent_id: user2.user_key)
      collection4_permission_template.reset_access_controls_for(collection: collection4, interpret_visibility: true)
      sign_in user2
      visit '/dashboard/my/collections'
    end
  ...
  describe 'create collection' do
    ...
    context 'when user can create collections of multiple types' do
      ...
      it "makes a new collection", :js do
        pending "Missing additional fields link"
  ...
  describe "adding works to a collection", skip: "we need to define a dashboard/works path" do
    let!(:collection) { FactoryBot.valkyrie_create!(:hyrax_collection, title: ["Barrel of monkeys"], user: user, with_permission_template: true) }
    let!(:work1) { FactoryBot.valkyrie_create(:monograph, title: ["King Louie"], depositor: user.user_key) }
    let!(:work2) { FactoryBot.valkyrie_create(:monograph, title: ["King Kong"], depositor: user.user_key) }
  ...
  describe 'delete collection', skip: "pending" do
    let!(:empty_collection) { FactoryBot.valkyrie_create(:hyrax_collection, :public, title: ['Empty Collection'], user: user, with_permission_template: true) }
    let!(:collection) { FactoryBot.valkyrie_create(:hyrax_collection, :public, title: ['Collection with Work'], user: user, with_permission_template: true) }
    let(:collection_permission_template) { Hyrax::PermissionTemplate.find_by(source_id: collection.id.to_s) }
    let!(:admin_user) { create(:admin) }
    let!(:empty_adminset) { FactoryBot.valkyrie_create(:hyrax_admin_set, title: ['Empty Admin Set'], creator: [admin_user.user_key], with_permission_template: true) }
    let!(:adminset) { FactoryBot.valkyrie_create(:hyrax_admin_set, title: ['Admin Set with Work'], creator: [admin_user.user_key], with_permission_template: true) }
    let!(:work) { FactoryBot.valkyrie_create(:monograph, title: ["King Louie"], admin_set_id: adminset.id, member_of_collection_ids: [collection.id.to_s], depositor: user.user_key) }
    ...
    context 'when user does not have permission to delete a collection' do
      ...
      before do
        create(:permission_template_access,
               :deposit,
               permission_template: collection_permission_template,
               agent_type: 'user',
               agent_id: user2.user_key)
        sign_in user2
  ...
  describe 'collection show page' do
    let!(:collection) do
      FactoryBot.valkyrie_create(:hyrax_collection, :public, user: user, description: ['collection description'], with_permission_template: true)
    end
    let!(:work1) { FactoryBot.valkyrie_create(:monograph, title: ["King Louie"], member_of_collection_ids: [collection.id.to_s], depositor: user.user_key) }
    let!(:work2) { FactoryBot.valkyrie_create(:monograph, title: ["King Kong"], member_of_collection_ids: [collection.id.to_s], depositor: user.user_key) }
  ...
  describe 'show pages of a collection' do
    ...
    let(:collection) { FactoryBot.valkyrie_create(:hyrax_collection, user: user, with_permission_template: true) }
  ...
  describe 'remove works from collection' do
    context 'user that can edit' do
      let!(:work2) { FactoryBot.valkyrie_create(:monograph, title: ["King Louie"], member_of_collection_ids: [collection1.id.to_s], depositor: user.user_key) }
      let!(:work1) { FactoryBot.valkyrie_create(:monograph, title: ["King Kong"], member_of_collection_ids: [collection1.id.to_s], depositor: user.user_key) }
    ...
    context 'user that cannot edit' do
      let!(:work1) { FactoryBot.valkyrie_create(:monograph, title: ["King Louie"], member_of_collection_ids: [collection3.id.to_s], depositor: user.user_key) }
      let!(:work2) { FactoryBot.valkyrie_create(:monograph, title: ["King Kong"], member_of_collection_ids: [collection3.id.to_s], depositor: user.user_key) }
  ...
  describe 'edit collection', skip: "pending" do
    ...
    context "navigate through tabs", js: true do
      let!(:empty_collection) { FactoryBot.valkyrie_create(:hyrax_collection, :public, title: ['Empty Collection'], user: user, with_permission_template: true) }
      let(:collection_type) { FactoryBot.create(:collection_type, :brandable, :discoverable, :sharable) }
      let(:collection_id) { FactoryBot.valkyrie_create(:hyrax_collection, user: user, collection_type_gid: collection_type.gid.to_s, with_permission_template: true).id }
      let!(:confirm_modal_text) { 'Are you sure you want to leave this tab? Any unsaved data will be lost.' }
      let!(:new_description) { 'New Description' }
