...
RSpec.describe ContentDeleteEventJob do
  ...
  context 'with a FileSet' do
    let(:curation_concern) { FactoryBot.valkyrie_create(:hyrax_file_set, title: ['Hamlet'], depositor: user.user_key) }

    it "logs the event to the depositor's profile" do
      expect { described_class.perform_now(curation_concern.id, user) }
  ...
  context 'with a Work' do
    let(:curation_concern) { FactoryBot.valkyrie_create(:hyrax_work, title: ['BethsMac'], depositor: user.user_key) }
