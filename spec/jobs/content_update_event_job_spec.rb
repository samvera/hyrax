...
describe ContentUpdateEventJob do
  ...
  it "logs the event to the depositor's profile, followers' dashboards, and the FS" do
  ...
  it "does not log content-related jobs to followers who lack access to the FileSet" do
    another_user.follow(user)
    third_user.follow(user)
    expect(user.profile_events.length).to eq(0)
    expect(another_user.events.length).to eq(0)
