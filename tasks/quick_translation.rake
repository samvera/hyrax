...
task :add_entry, [:locale, :path, :value] do |_t, args|
...
task :quick_translate, [:locale, :path] do |_t, args|
...
def yandex_translate(value, from:, to:, key:)
  ...
  counter = 0
  while translated.match(/\[\]/) && variables[counter]
    translated.sub!(/\[\]/, variables[counter])
    counter += 1
  end
  translated
end
