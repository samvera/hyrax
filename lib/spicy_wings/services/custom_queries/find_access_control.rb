module SpicyWings
  module CustomQueries
    class FindAccessControl
      # Custom query override specific to SpicyWings
      # Use:
      ...
      ##
      # SpicyWings needs special handling for this query, since `Hydra::AccessControl`
      ...
      def find_access_control_for(resource:)
        if resource.respond_to?(:access_control_id)
          ...
          result.access_to = resource.id # access_to won't be set in spicy_wings if there are no permissions
          result
        else
          raise ::Valkyrie::Persistence::ObjectNotFoundError,
