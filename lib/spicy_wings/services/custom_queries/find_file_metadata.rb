# Custom query override specific to SpicyWings for finding Hydra::PCDM::File and converting to Hyrax::FileMetadata.
# @example
  ...
  module CustomQueries
    class FindFileMetadata
      ...
      def find_file_metadata_by_alternate_identifier(alternate_identifier:, use_valkyrie: true)
        ...
        SpicyWings::FileConverterService.af_file_to_resource(af_file: object)
      end
      ...
      def find_many_file_metadata_by_ids(ids:, use_valkyrie: true)
        ...
        ids.each do |alt_id|
          begin
            # For SpicyWings, the id and alt_id are the same, so just use alt id querying.
