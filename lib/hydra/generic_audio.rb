...
#   Included from Hydra::GenericAudio: max, thumbnail, screen
#
# Sample Usages:
#   For good sample usage, see the documentation for HydraImage.
require "hydra"

module Hydra::GenericAudio
  def self.included klass
    klass.send(:include, Hydra::GenericContent)
  end

  class Hydra::GenericAudio::NoFileError < RuntimeError; end;
  class Hydra::GenericAudio::UnknownAudioType < RuntimeError; end;

  DEFAULT_AUDIO_DATASTREAMS = ["max","thumbnail","screen"]
  ...
  DEFAULT_AUDIO_DATASTREAMS.each do |ds_name|
    class_eval <<-EOM
      def has_#{ds_name}?
  ...
  def derive_all
    DEFAULT_AUDIO_DATASTREAMS.each { |ds| self.send "derive_#{ds.to_sym}" }
  end
  ...
  def delete_derivatives
    DEFAULT_AUDIO_DATASTREAMS.each { |ds| datastreams[ds].delete if datastreams.has_key? ds }
  end
