...
class ModsImage < ActiveFedora::NokogiriDatastream       
  ...
  set_terminology do |t|
    ...
    def insert_contributor(type, opts={})
      case type.to_sym 
      when :person
        node = Hydra::ModsImage.person_template
        nodeset = self.find_by_terms(:person)
      when :organization
        node = Hydra::ModsImage.organization_template
        nodeset = self.find_by_terms(:organization)
      when :conference
        node = Hydra::ModsImage.conference_template
        nodeset = self.find_by_terms(:conference)
      else
        ActiveFedora.logger.warn("#{type} is not a valid argument for Hydra::ModsImage.insert_contributor")
        node = nil
        index = nil
      end
